...
\end{wrapfigure}%
Most medicine students learn about the \emph{base-rate fallacy} \citep{barhillel1980,jennyetal2018,sprengeretal2021,matthews1996}. Suppose that in a large set of clinical trials it is found that among patients having a particular value of some predictors, 71.4\% (or 5/7) eventually developed a disease, while 28.6\% (or 2/7) did not. See upper panel of side figure; each dot may represent 10\,0000 or one million samples. The fallacy lies in judging that a new real patient from the full population, who has those particular predictor values, also has a 71.4\% probability of developing that disease. In general this probability will instead be different. In the side-figure example, lower panel, it is 33.3\%. This discrepancy arises from the fact that the proportion of positive vs negative disease cases in the full population has some value, 16.7\%/83.3\% in the side example; but the samples for the trials were chosen so as to have a roughly 50\%/50\% proportion, as indicated by the dashed line. This procedure is called \enquote{class balancing} in machine learning. More generally this discrepancy can appear whenever a population and a sample dataset from it do not have the same frequency distribution for the disease or predictand.

A little counting in the side figure reveals, however, that \emph{in the full population} among all patients who developed the disease, 83.3\% (or 5/6) of them had the particular predictor value, while among those who did not develop the disease, 33.3\% (or 1/3) had the particular predictor value -- \emph{and these frequencies are the same in the clinical trials}. These frequencies can therefore be used to make a prognosis using Bayes's theorem:
\begin{equation}
