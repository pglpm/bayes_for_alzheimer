...
The present work presents an assistive and statistically rigorous algorithm--the \ljm-- for personalized medicine.
This algorithm is designed to first learn from a dataset of clinical and supplementary data with relevant predictors and predictand, and then assist a clinician in the assessment of prognosis \& treatment for new patients. Max amount of information... We apply the \ljm in a realistic setting for clinical decision-making, incorporating clinical, environmental, imaging, and genetic data, using a data set of subjects suffering from mild cognitive impairment and \ad. We discuss some major advantages of this model-free approach for decision-making under risk, resource planning, imputation of missing values, assessing the importance of each predictor in the prognosis, and more. 
...
In actual deployment, we would recommend the hospital, medical centre, or clinician using a \ljm\ to keep a database of incoming patients, with their predictor values, adding the true values of the predictand later in time, once they become known. The \ljm\ can then be retrained on such local database, when the latter reach the size of the original one. All inferences would thus become even more reliable, because based on the local statistics and population characteristic of that hospital.
