...
\begin{table}[!b]  
  ...
  \end{center}
    ...
    \item\label{item:population} Assess which statistics of the dataset can be applied to the present patient, and any auxiliary clinical information available. Quantify the latter it in a prior probability.

      Input the patient's  clinical data, relevant statistics, and auxiliary information into the \tjm. 

      Upon request, the machine can output the final probability of the predictand's true value for the patient and any other probabilities and likelihoods of interest.
    ...
    \item\label{item:utilities} Assess the clinical courses of action (treatments, more tests, and so on) available for the present patient, and the utility (benefit and loss) of each course of action, depending on each possible predictand value for the patient.
      ...
      Upon request, the machine can output the probability of gaining different utilities, perform sensitivity analyses for missing data, and similar tasks.
    \end{enumerate}
  \end{framed}
\end{table}
