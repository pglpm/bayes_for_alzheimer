...
Through four fictional (but statistically realistic) patients we have shown that the differences among patients which are relevant to clinical diagnosis or prognosis and decision-making for a specific medical condition, can be approximately divided into three categories:
\begin{enumerate}[I.]
...
\item\label{item:diff_priorinfo} differences in the availability and values of auxiliary and usually \enquote{softer} predictors, such as geographical or family background; or more generally in predictors not belonging to the core set;
