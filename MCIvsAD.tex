...
Patients with Mild Cognitive Impairment (MCI) have an increased risk of being at a trajectory towards Alzheimer's Disease (AD). 
%Early identification of patients with a high risk of Alzheimer's Disease is essential to provide treatment 
%before the disease is well-established in the brain, is an important endeavour. great importance to study 
To identify how well different kinds of features can contribute to correctly predict this trajectory in individual patients is an important endeavour. Although this would be important for prognosis, prevention and treatment, 
%-- from neuropsychological examinations to advanced brain-imaging techniques -- 
%from Mild Cognitive Impairment towards Alzheimer's Disease in an individual patient.
%more is needed for a personalized approach to prognosis, prevention, and treatment, than just the obvious requirement that prognoses be as best as they can be for each patient. 
several situational elements 
%that can be different from patient to patient 
must be accounted for, such as:
\begin{itemize}
...
\end{itemize}
Statistical decision theory is a normative quantification framework that takes into account these fundamental differences. Medicine has the distinction of having been one of the first fields to adopt this framework, exemplified in brilliant old and new textbooks on clinical decision-making.
%Clinical decision-making 
This framework makes allowance for these differences among patients through two requirements. First, the quantification of prognostic evidence on one side, and of benefits and costs of treatments and prevention strategies on the other, must be clearly separated and handled in a modular way. Two patients can have the same prognostic evidence and yet very different prevention options. Second, the quantification of independent prognostic evidence ought to be in the form of \emph{likelihoods about the health condition} (or equivalently of likelihood ratios, in a binary case), that is, of the probabilities of the observed test outcomes given the hypothesized health conditions. Likelihoods from independent clinical tests and predictors can then be combined with a simple multiplication; for one patient, we could have three kinds of predictor available; for another, we could have five. The clinician's pre-test assessment is included in the form of a probability. These patient-dependent probabilities are combined with the patient-dependent costs and benefits of treatment or prevention to arrive at the best course of action for that patient. The main result underlying statistical decision theory is that decision-making \emph{must} take this particular mathematical form in order to be optimal and logically consistent.

The present work investigates the prognostic power of a set of measures from neuropsychological and Magnetic Resonance Imaging examinations, demographic data, and genetic information about Apolipoprotein-E4 status
%The present work investigates the prognostic power of a set of neuropsychological and Magnetic Resonance Imaging examinations, demographic data, and genetic information about Apolipoprotein-E4 (APOE) status, 
for the prediction of the onset of Alzheimer's Disease in patients defined as mildly cognitively impaired at a baseline examination. The longitudinal data are selected from the ADNI database.
...
%. From these results, available clinical statistical data, and other relevant information, their clinician will assess their risk of developing Alzheimer. 
...
We can categorize these differences as \enquote{difference in auxiliary information} (Olivia and Ariel), \enquote{difference in preventive benefits} (Olivia and Bianca), \enquote{difference in clinical predictors} (Olivia and Curtis). Table~\ref{tab:patients_data} reports the clinical results and demographic data common to Olivia, Ariel, Bianca, as well as those of Curtis \mynotew{need to explain the variates and refer to \citep{ryeetal2022}}. The figure on its side summarizes the similarity and differences between Olivia and the other three patients on the clinical examinations:Hippocampus volume (HC); American National Adult Reading Test (ANART); Category Fluency Test (CFT); GDS: Geriatric Depression Scale; Rey- Auditory Verbal Learning Test (RAVLT); -im: immediate recall; -del: delayed recall; -rec: recognition; Trail Making Test version A (TMTA) and B (TMTB)\mynotew{need some more space in a Method section to describe more details about the tests. Inspired from  \citep{ryeetal2022}}.. 
...
% ##     olivia    ariel   curtis
% ## 1 0.698321 0.527060 0.309080
% ## 2 0.718993 0.616236 0.485448
% ## 3 0.709496 0.658118 0.592724
% ## 4 0.301679 0.472940 0.690920
...
% MI: 
%                               mean     sd
% GDTOTAL_gds               0.000521 0.0003
% Gender_num_               0.003200 0.0008
% Apoe4_                    0.003500 0.0010
% AGE                       0.006160 0.0010
% ANARTERR_neuro            0.006860 0.0010
% TRAASCOR_neuro            0.013300 0.0020
% TRABSCOR_neuro            0.023900 0.0020
% CATANIMSC_neuro           0.024300 0.0020
% LRHHC_n_long              0.028100 0.0020
% genetic-demo vars         0.036000 0.0030
% AVDELTOT_neuro            0.060300 0.0030
% RAVLT_immediate           0.092400 0.0040
% AVDEL30MIN_neuro          0.094000 0.0040
% allminus_AVDEL30MIN_neuro 0.125000 0.0040
% allminus_RAVLT_immediate  0.131000 0.0040
% allminus_TRABSCOR_neuro   0.135000 0.0040
% cognitive vars            0.137000 0.0040
% allminus_AVDELTOT_neuro   0.138000 0.0040
% allminus_LRHHC_n_long     0.138000 0.0040
% allminus_TRAASCOR_neuro   0.139000 0.0040
% allminus_CATANIMSC_neuro  0.139000 0.0040
% allminus_AGE              0.139000 0.0040
% allminus_GDTOTAL_gds      0.140000 0.0040
% allminus_ANARTERR_neuro   0.140000 0.0040
% allminus_Gender_num_      0.140000 0.0040
% allminus_Apoe4_           0.140000 0.0040
% all                       0.140000 0.0040
...
\mynotep{Add brief explanation of mutual information, then summarize interesting results:}

From table~\ref{tab:mutual_info} we see that, \emph{within the population selected for the dataset}:
\begin{itemize}
\item The presence of the APOE4 allele is a poor predictor, both when used alone and when used in combination with all other predictors.
\item The combined cognitive variates are better predictors than the combined genetic-demographic ones.
\item Hippocampal volume is au pair with most cognitive-test predictors, when used alone, but not very important in combination with all other predictors.
\item The \textsf{RAVLT-imm}, \textsf{RAVLT-del}, and to a lesser degree the \textsf{RAVLT-rec} tests are important predictors, both when used alone and when used in combination with all other predictors.
\end{itemize}
...
LRHHC-n-long              &0.028  \\
\textbf{genetic-demog vars}         &0.036  \\
...
As mentioned in \sect~\ref{sec:learning_step} and explained in appendix~\ref{sec:maths_tjm}, 
