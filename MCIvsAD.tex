...
The \tjm\ also reveals that the present dataset does not allows us to precisely quantify the decrease in prognostic power when one of the 12 predictors is omitted. An analysis -- not reported here owing to space constraints \mynotez{if there's time I'll try to include these variability ranges} -- about the possible variability of the results of \fig~\ref{fig:mutual_info} if a larger dataset were available, shows that the relative ordering of the mutual information of the \enquote{all minus $X$} predictor sets could easily change. (It is thus probable that the importance analysis of \citealp{ryeetal2022} only reflects peculiarities of the present dataset rather than properties generalizable to the full population; this is also plausible considering that the methods employed there are of the bootstrap kind \mynotew{***add ref about this, maybe \citep[\chap~10]{efron1982_r1994}}.)

\medskip

Regarding Curtis's missing value for Hippocampal volume, the clinician notices that acquiring it would only increase the prognostic accuracy by less than 0.5\%, and so decides to proceed without it.
...
\begin{table}[!t]
  ...
  \begin{tabular}{lcccc}
    ...
    &
    $\begin{matrix}3.09\\4.85\\5.93\\\bm{6.91}\\[2\jot]\bm{\delta}\end{matrix}$
    \\[5\jot]
    \hline
  \end{tabular}
