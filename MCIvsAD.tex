...
Patients with Mild Cognitive Impairment have an increased risk of a trajectory toward Alzheimer's Disease. To provide efficient treatment, it is essential to identify this impairment at an early stage, before the disease is too well-established in the brain. Clinicians still tend to find it difficult to make decisions regarding 
%-- from neuropsychological examinations to advanced brain-imaging techniques -- 
the probability of a conversion from an MCI to an AD diagnosis in an individual patient. In addition to results from a clinical examination, s/he must take 
%iallow us to prognose a trajectory from Mild Cognitive Impairment towards Alzheimer's Disease in an individual patient.
 several situational factors must be taken into account to obtain a personalized approach to prognosis, prevention, and treatment. 
...
\end{itemize}
Statistical decision theory is the normative quantification framework that takes into account such fundamental differences between patients. Medicine has the distinction of having been one of the first fields to adopt this framework, exemplified in brilliant old and new textbooks on clinical decision-making.
...
The present study uses different statistical approached to investigate the prognostic power of results from neuropsychological and Magnetic Resonance Imaging examinations, demographic data, and genetic information about Apolipoprotein-E4
...
Meet Olivia, Ariel, Bianca, and Curtis.\footnote{Fictive characters; any reference to real persons is purely coincidental}
% names from Shakespeare's plays
These four persons don't know each other, but they have something in common: they all suffer from a mild form of cognitive impairment, and are afraid that their impairment will turn into Alzheimer's Disease within a couple of years. In fact, this is why they recently asked for a clinical examination, including a set of cognitive tests, MRI and genetic analysis. Today they received the results from this examination. Based on these results, available clinical statistical data, and other relevant information, their clinician 
% indicate % [what does this mean?]
informed the patients about their risk of developing Alzheimer, and then made a decision among a set of mutually exclusive preventive-treatment options together with the patients and their relatives.

Besides a shared MCI diagnosis and associated worries, these patients have other things in common as well as some differences. Let's take Olivia as reference and list the similarities and difference between her and the other three patients:
\begin{itemize}
\item Olivia and Ariel turn out to have exactly identical clinical results and age. They would also get similar benefits from the available preventive-treatment options. Ariel, however, comes from a geographical region with a higher rate of conversion from MCI to \ad, and she had a much heavier family %history . Moreover, Ariel comes, unlike Olivia,from a family with a heavy 
history of Alzheimer's Disease. 
Because of this geographical and family background, 
From this information, the clinician judges a priori a 65\% probability that Ariel's cognitive impairment will convert to Alzheimer's Disease

\item Olivia and Bianca also have exactly the same clinical results and age. They come from the same geographical region and have very similar family histories. In fact we shall see that they have the same probability of developing Alzheimer's disease. Bianca, however, suffers from several allergies and additional somatic diseases that render some of the preventive options slightly riskier for her.

\item Olivia and Curtis have different results on all neuropsychological tests, all in disfavour of Curtis. Curtis is also the only patient with the risky Apolipoprotein-E4 (APOE4) allele \citep{liuetal2013}.
%whereas Curtis has. 
Olivia is also more than 10 years older than Curtis. They otherwise come from the same geographical region, have very similar family histories, and is expected to get similar benefits from the preventive options. Note that one clinical result of Curtis's (hippocampal volume) is missing.
\end{itemize}
...
Considering the similarities and differences among these patients, which treatments are optimal and should be prescribed?
...
Given these predictor values the \tjm\ yields the following probabilities of main importance for the  application in the next sections:
...
% "coverage interval": \citep[C.2.30]{jcgm1993_r2008}
\begin{equation}
...
The expected accuracy and mutual information (on a 0--1 scale) for these sets are reported in \fig~\ref{fig:mutual_info}, ordered from bottom to top according to increasing mutual information. The ordering of mutual information and accuracy agree within the numerical computation error. The latter is reported as bars of $\pm$~one standard deviation.
\begin{figure}[!t]% with figure
  \centering%
\includegraphics[width=0.494\linewidth]{accuracy_ranks.pdf}%
\hfill%
\includegraphics[width=0.494\linewidth]{MI_ranks.pdf}%
\caption{Expected accuracy and mutual information of several sets of predictors for the prognosis of conversion to \ad. Both graphs have been vertically ordered according to increasing values; the two rankings agree within the respective uncertainties. The \textsf{all} predictor set is mathematically guaranteed to be optimal according to both metrics, and has therefore be ranked first. Bars show the numerical computation error ($\pm$~two standard deviations).}\label{fig:mutual_info}
\end{figure}%
...
\mynotep{Difficulty in assessing and quantifying additional info: Just ignoring it is not a solution and is unethical}
