...
\subsection{Underlying principles and characteristics}
...
\item \emph{Regression or classification:} the average value of any set of variates $Y$, given any other set of variates $X$, including the special case of predictand $Y$ and predictors $X$. The uncertainty or variability around such average is also automatically computed.
...
The second factor is a prior probability expressing how we expect reasonable candidates to be. The only natural requirement is that it should have some degree of smoothness, owing to physical and biological reasons. Figure~\ref{fig:prior_distribution} shows, through a sample, what the machine considers to be \enquote{reasonable candidates} for the population frequency distribution of a discrete variate. 
\begin{figure}[b]% with figure
  \centering%
  \includegraphics[width=0.75\linewidth]{priorexamples_AVDEL30MIN_neuro.pdf}
  \caption{\mynotep{Examples probable candidates of frequency distribution for a variate such as \ravltdel\ %AVDEL30MIN\_neuro
...
The predictor values for our four patients are reported in table~\ref{tab:patients_data}. Note that Curtis's value for the Hippocampal Volume is missing; the \ljm\ has no difficulty with this.
\begin{table}[!b]
  \centering
...
\begin{equation}
  ...
  % \p(\texttt{\small predictand} \| \texttt{\small predictors}) \propto
...
\begin{figure}[!t]% with figure
  \centering%
  \includegraphics[width=0.75\linewidth]{priorexamples_AVDEL30MIN_neuro.pdf}
  \caption{\mynotep{Examples of a-priori probable candidates of frequency distribution for a variate such as \ravltdel%AVDEL30MIN\_neuro
 or \ravltrec%AVDELTOT\_neuro
}}\label{fig:prior_distribution}
\end{figure}%
