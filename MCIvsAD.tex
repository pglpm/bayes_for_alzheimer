...
The inferential and decision-making steps are summarized in table~\ref{tab:inference_decision_steps}. Steps in \textbf{boldface} represent especially important patient-dependent, personalized steps.
\begin{table}[!h]
  \centering
  \begin{framed}
    \caption{Inferential and decision-making steps}
    ...
    \item\label{item:likelihood}Calculate the \emph{likelihood} -- as opposed to the probability -- of the predictand for a specific patient, given the patient's predictor values. Combine this likelihood with the prior from step~\ref{item:prior}, to obtain the final probability for the predictand, for the present patient.
    ...
    \end{enumerate}
  \end{framed}
  \label{tab:inference_decision_steps}
\end{table}
