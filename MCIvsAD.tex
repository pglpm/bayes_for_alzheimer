...
Our main point here is that population variability and auxiliary clinical information are important factors that differentiate patients, and a personalized approach ought to take them into account. The method presented here does this naturally, allowing a great flexibility in selecting which statistical features of the sample dataset should be used for each new patient, and the integration of auxiliary clinical information in the form of a prior probability. As discussed in \sect~\ref{sec:learning_step}, the ideal machine-learning algorithm used here allows us to quickly calculate conditional probabilities $\p(Y\|X, \textsf{\small dataset})$ for any desired variate subsets $Y$ and $X$ required by the patient's relevant population.
...
In our present example, all statistics of the dataset are considered relevant for Olivia, Bianca, and Curtis. For these patients we therefore use the probabilities $\p(\textsf{\small conversion} \| \textsf{\small predictors}, \textsf{\small dataset})$, having values
\begin{equation}
  \label{eq:posterior_probs}
  p_{\text{Olivia}} = p_{\text{Bianca}} = 0.30\ ,\qquad
  p_{\text{Curtis}} = 0.70\ .
\end{equation}

For Ariel, however, the clinician judges that a different, 65\% base rate of conversion should be used, owing to her different geographical origin and family history. In her case we must use the probabilities $\p(\textsf{\small predictors} \| \textsf{\small conversion}, \textsf{\small dataset})$ and $\p(\textsf{\small predictors} \| \textsf{\small no\,conversion}, \textsf{\small dataset})$, calculated to be
\begin{equation}
  \label{eq:ariel_likelihoods}
  l_{\text{conv.}} = 8.97\cdot10^{-12} \ , \qquad
  l_{\text{no\,conv.}} = 1.86\cdot10^{-11} \ .
\end{equation}
Combining these in Bayes's theorem we obtain the conversion probability
\begin{equation}
  \label{eq:ariel_probability}
  p_{\text{Curtis}} = 0.47 \ .
\end{equation}
Note how this probability is higher than for Olivia and Bianca, even if the predictor data are the same for these three patients.



\mynotew{Illustration of the advantage of using likelihoods: We split our learning dataset in two subsets:
  \begin{itemize}
  ...
  First we use the probabilities $\p(\textsf{\small predictand} \| \textsf{\small predictors}, \textsf{\small dataset})$ for making prognoses on the \enquote{full population} of 343 patients, considering the specific predictor values of each one. We obtain 37.3\% correct prognoses.

  Then we use the probabilities $\p(\textsf{\small predictors} \| \textsf{\small predictand}, \textsf{\small dataset})$ instead, together with the base rate of 63.3\%, according to \eqn~\eqref{eq:base-rate_correction}. We obtain 73.2\% correct prognoses.

  Consider that if we made prognoses on the \enquote{full population} of 343 patients by a coin toss, we would obtain 50\% correct prognoses. Always prognosing \enquote{stable \mci}, we would obtain 63.3\% correct prognoses. The first approach leads to results worse than a coin toss because of the underlying base-rate fallacy. The second approach, instead, leads to better results than just always prognosing the most common outcome, because it combines appropriate statistical information from the learning dataset with the correct base rate.}
