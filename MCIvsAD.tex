...
(the four corresponding probability histograms, if plotted jointly, would look like distinct vertical lines). It is clear that knowledge of the Hippocampal Volume is extremely unlikely to change Curtis's optimal treatment from $\delta$. 
% old text:
%Considering the economic costs (involving an \textsc{mri}-scan) for obtaining this uninformative predictor, the clinician decides to proceed without it.
% [alternative:]
Considering that the negligible information gained would not outweigh the economic costs (involving an \textsc{mri}-scan) for obtaining this predictor, the clinician decides to proceed without it.
