...
The \ljm\ took less than six hours to calculate the probability distribution for the possible population joint frequencies distributions of the 13 variates. Some results can already be visualized after this inference. Figure~\ref{fig:marginal_pop_distributions} shows the predicted  distributions of \ravltdel\ and for hippocampal volume, for the subpopulation of patients that will convert to Alzheimer (red dashed) and the subpopulation that will remain with stable \mci\ (solid blue). The thin curves are 100 samples of highly probable frequency distributions; the thicker lines are their means, which are also the predictive conditional probabilities for a hypothetical new patient.
\begin{figure}[b]
\centering%
...
\caption{Predicted distributions of two predictor variates, for the subpopulation of patients that will convert to Alzheimer (red dashed) and the subpopulation with stable \mci\ (solid blue).}\label{fig:marginal_pop_distributions}
\end{figure}%

The two subpopulations of patients are clearly distinct in each of these two variates. The learning dataset contains enough data to greatly reduce our uncertainty about the population distribution, as evident from the very narrow spread of the curves.
