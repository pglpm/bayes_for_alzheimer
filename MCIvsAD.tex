...
The present work presents a statistically sound and rigorous algorithm 
%rigorous
% sound
% principled
% first-principled
...
% ...
-- the \ljm\ -- for use in the context of personalized medicine.
The \ljm\ is designed to first learn from a dataset of clinical and supplementary data with relevant predictors and predictand, and then assist a clinician in the assessment of prognosis \amp\ treatment for new patients. We apply the algorithm in a realistic setting for clinical decision-making, incorporating clinical, environmental, imaging, and genetic data, using a data set of subjects suffering from mild cognitive impairment and \ad. We show how the algorithm is theoretically optimal and discuss some major advantages of this model-free approach for decision-making under risk, resource planning, imputation of missing values, assessing the importance of each predictor in the prognosis, and more. 
...
\begin{equation*}
\begin{gathered}
\p(\textrm{not-}A \| X) = 1-\p(A\|X)\\
\p(A\mathbin{\textrm{or}}B \|X) =
\p(A \|X) + \p(B \|X) -
\p(A\mathbin{\textrm{and}}B \|X) \\
\p(A\mathbin{\textrm{and}}B \|X) =
\p(A \| B\mathbin{\textrm{and}} X) \,
\p(B \|X) \ .
\end{gathered}
\end{equation*}
...
Clinical data included in this article were obtained from the Alzheimer’s Disease Neuroimaging Initiative (ADNI) database (http://adni.loni.usc.edu). A complete listing
