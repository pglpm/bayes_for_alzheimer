...
As discussed in \sect~\ref{sec:learning_step}, the \tjm\ explores the space of possible distributions of frequencies of all variates, for the \emph{full population} of patients. In the present study it does so by using 191 parameters for each continuous or integer variate (binary variates only require one parameter). The total number of parameters is 1536. As a crude intuition, it is as if we divided the range of each variate into 191 bins, and considered all possible frequency histograms over these. The actual parametrization of the frequency space is more subtle than this. The data actually indicate that fewer than 30 parameters per variate would suffice.
