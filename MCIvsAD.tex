...
Let us comment on some of these features. We believe that the capability of working with complex predictands, feature~\ref{item:feat_variates}, is important for a more realistic and nuanced approach to prognosis. In the case of \ad, for instance, a simple dichotomy \enquote{has disease} vs. \enquote{doesn't have disease} is possibly an oversimplification\footnote{see \eg\ \citealp{edmondsetal2015,edmondsetal2020},  whose methods we find, however, inconclusive}.}.
% [Luca:] They standardize neuropsych. scores based on age and education "based on regression coefficients derived from the normal control group" -- this can already introduce terrible biases and unrealistic approximations (did they check if linearity holds?). Then the rest is the usual astrology with Bonferroni corrections etc. The graphs in their paper don't really tell me anything. In Fig.~3 of the 2020 paper I would have liked to see the points, not just the linear-regression lines. The fact that they omitted the points may actually be a sign that they would have shown the linear regression to be unrealistic.
