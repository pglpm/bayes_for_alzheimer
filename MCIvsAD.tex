...
The present method moreover yields additional useful information:
\begin{itemize}
...
\emph{Neural networks} are based on the assumptions that there is a \emph{functional relationship} from the predictor to the predictand,
...
% \end{wrapfigure}%
possibly contaminated by a little noise (typically assumed gaussian). This is a very strong assumption, quite unrealistic for many kinds of variables considered in medicine. It can only be justified in the presence of informationally very rich predictors, such as images. In our case, for example, the mutual information between predictors and the conversion variate is $0.16\,\mathrm{Sh}$, to be compared with $1\,\mathrm{Sh}$, if conversion were a function of the predictors, and with $0\,\mathrm{Sh}$, if conversion were completely unpredictable. See \sect~\mynotep{***} for further details.




In the frequency distribution depicted on the right, for example, a neural network could be acceptable to represent the range $X < 15$, but would be hopeless for $X > 17$. Neural networks, moreover, typically output only the expected value of the predictand, $\E(Y \| X)$, not the full conditional probability $\p(Y \| X)$. \mynotep{Add references and generalizations.}
...
%%%% MI given all minus ...
% >    AVDEL30MIN_neuro RAVLT_immediate LRHHC_n_long TRABSCOR_neuro AVDELTOT_neuro
% mean            0.147           0.157        0.160          0.160          0.162
% sd              0.004           0.005        0.005          0.005          0.005
%      TRAASCOR_neuro Apoe4_   AGE CATANIMSC_neuro GDTOTAL_gds Gender_num_
% mean          0.163  0.163 0.164           0.164       0.164       0.165
% sd            0.005  0.005 0.005           0.005       0.005       0.005
%      ANARTERR_neuro   all
% mean          0.165 0.165
% sd            0.005 0.005
%%%% differences
% >      all ANARTERR_neuro Gender_num_ GDTOTAL_gds CATANIMSC_neuro     AGE  Apoe4_
% mean 0.000       0.000109    0.000423    0.000941        0.000967 0.00102 0.00192
% sd   0.009       0.009000    0.009000    0.009000        0.009000 0.00900 0.00900
%      TRAASCOR_neuro AVDELTOT_neuro TRABSCOR_neuro LRHHC_n_long RAVLT_immediate
% mean        0.00247        0.00337        0.00508      0.00531         0.00795
% sd          0.00900        0.00900        0.00900      0.00900         0.00900
%      AVDEL30MIN_neuro
% mean           0.0186
% sd             0.0090
